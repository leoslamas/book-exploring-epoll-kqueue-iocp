...
Let's dive in and first have a look at the `ffi`part of our code:

Crate a new rust `bin` project \(not a library\). In `main.rs`remove what's there and add:
...
Now, let's take a look at the code. I've commented it extensively to explaing along the way but will point out some interesting parts below.
